...
import os
import sys  # PC backend secimi icin eklendi
...
# PySerial (PC'de kurulu degilse simulasyon moduna dus)
try:
    import serial
except Exception:
    serial = None
...
# ==============================================================================
# MULTI-THREADED CAMERA CLASS (LIBCAMERA / GSTREAMER + PC WEBCAM)
# [1. KOD KORUNDU, PC UYUMLULUK EKLENDI]
# ==============================================================================
class VideoStream:
    def __init__(self, src=0, width=640, height=480):
        ...
        # Pi/libcamera modu (orijinal davranis)
        if Config.USE_PI_GSTREAMER:
            gst_pipeline = (
                "libcamerasrc ! "
        ...
        # PC/laptop webcam modu (eklenen uyumluluk)
        else:
            print(f"[INFO] PC kamera aciliyor... source={src}")
            ...
            if self.stream is not None:
                # Webcam ise uygulanir, video dosyasiysa etkisiz olabilir
...
# ==============================================================================
# CONFIGURATION (1. KOD + 2. KOD AYARLARI BIRLESTIRILDI)
# PC UYUMLULUK ICIN SADECE GEREKLI EKLER + YOLO STEER IYILESTIRME
# ==============================================================================
class Config:
    # Camera
    FRAME_WIDTH = 640
    ...
    # PC/Pi kamera secimi (EKLENDI)
    USE_PI_GSTREAMER = False   # PC'de False, Pi'de True
    CAMERA_SOURCE = 0          # PC webcam index (0/1) veya video dosyasi yolu
    ...
    # PC testte Pico yoksa YOLO'yu her frame calistir (EKLENDI)
    ...
    # Not:
    # - Artik steer siddeti sadece sol/sag farktan degil, nesnenin merkeze yakinligindan da etkileniyor.
    # - Bu ayarlar "hafif sagda" ile "en sagda" arasinda farki daha gorunur yapar.
    DEADBAND = 0.10
    SWITCH_HYS = 0.30
    GAIN = 1.15
    STEER_MIN = 0.06
    ...
    # Pi4 UART (GPIO14/15) / PC serial (EKLENDI)
    SERIAL_ENABLED = False  # PC'de False (simulasyon), Pico baglarsan True
...
# PC uyumluluk: pyserial yoksa veya SERIAL_ENABLED=False ise simulasyon modunda calis
if Config.SERIAL_ENABLED and serial is not None:
    try:
    ...
    PICO_BAGLI = False
    if serial is None:
        print("[WARNING] pyserial bulunamadi - SIMULASYON MODU")
    else:
        print("[INFO] SERIAL devre disi - PC TEST MODU")
...
# ==============================================================================
def main():
    print("[INFO] KAMERA BASLATILIYOR (PC/PI UYUMLU MOD)...")
    try:
        vs = VideoStream(src=Config.CAMERA_SOURCE,
                         width=Config.FRAME_WIDTH,
                         height=Config.FRAME_HEIGHT).start()
    except Exception as e:
    ...
    if vs.stopped:
        print("[FATAL] Kamera baslatilamadi. Kamera index/backend kontrol et.")
        return
    ...
    try:
        # Orijinal davranis korunur: lokal dosya varsa custom path ile yukle
        # PC uyumluluk: dosya yoksa pretrained yolov5n fallback
        ...
        try:
            ...
            model.to(device)
            print(f"[INFO] YOLO hazir. Device: {device}")
        except Exception:
    ...
    try:
        # PC'de pencereyi buyutulebilir yapalim (ozellik degistirmez)
        cv2.namedWindow("TEMIRBUG PRO HUD + YOLO", cv2.WINDOW_NORMAL)
        ...
        while True:
            ...
            # ------------------------------------------------------------------
            # PC test modunda Pico yoksa YOLO'yu yine de calistir ki detection gorunsun
            ...
            # 2. koddaki near obstacle çizgisi (YOLO gerekli/gerekmez gösterilebilir)
            ...
            if mesafe_cm <= Config.STOP_CM:
                ...
                # Sadece olcek degisti: yolo_steer (±STEER_MAX) -> angle (±90)
